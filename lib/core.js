...
function defaultConf (scheme) { 
  return (scheme == null
    ? { backslashes:true, drive:false }
    : { backslashes: scheme in specialSchemes, drive: scheme === 'file' }) }
...
function parse (input, conf_) {
  ...
  let { backslashes, drive } = conf (scheme) 
    ...
    , beforeQuery = true // check for query delimiter (?)
    , beforeHash  = true // check for fragment delimiter (#)
  ...
  function emit (type, value) {
    if (drive && (type === AUTH || type === DIR || type === FILE) && R_DRIVE.test (value)) {
      ...
      if (last === ROOT) r.pop ()
      r.push ([DRIVE, value])
      if (type === DIR) r.push ([ROOT, '/'])
      drive = false
    }

    else if (type === AUTH)
      r.push ([type, parseAuth (value)])

    else if (type !== FILE || value.length) {
      r.push ([type, value])
      drive = drive && compareType (type, DIR) < 0
      // DIR because, ROOT, (DIR|FILE) may be converted to DRIVE
      // Hence, once something >= DIR has been emitted the check stops
    }
  ...
  while (p < input.length) {
    ...
    else if (c === '?' && beforeQuery) {
      emit (part, buff)
      part = QUERY
      _auth = _dir = beforeQuery = false
    }

    else if (c === '#' && beforeHash) {
      emit (part, buff)
      part = FRAG
      _auth = _dir = beforeQuery = beforeHash = false
    }

    else if (c === '\0' && !beforeHash) {
...
function cursor (url, i, l, type, value = null) {
  ...
  return { found:!!l, type, value, get, set, prev, next }
}
...
// var u = parse ('http://localhost/d1/d2/f?q#foo')
// log (u)
// log (select (u, SCHEME) .value)
// log (select (u, AUTH) .value)
// log (select (u, DRIVE) .value)
// log (select (u, ROOT))
// log (select (u, DIR))
// log (select (u, FILE))
// log (select (u, DIR).set())
// log (select (u, FILE).set('file') )
// log (select (u, DIR).set ())
// var u = parse ('http://a#frag')
// log (u)
// log (select (u, ROOT))
// log (select (u, QUERY))
