...
function parse (input, mode) {
  ...
  if (part !== FILE || buff) r.push ([part, buff])
  r .forEach (_ => { if (_[0] === AUTH) _[1] = auth.parse (_[1]) }) // TODO do this more cleanly
  return r
}
...
function parseAuth (string) {
  ...
  const auth = { _auth:string, name:null, pass:null, host:'', port:null }
...
const _emptyAuth = { _auth: '', name: null, pass: null, host: '', port: null }
...
// Else if the first DIR or FILE is a drive letter, demote it to DRIVE, ROOT, resp. DRIVE.

function letter (url) {
  const r = []
  let done = false // DRIVE token created, or no drive detected. 

  for (let i=0, l=url.length; i<l; i++) {
    const token = url [i]
      , value = token [VALUE]
      , last = i > 0 ? url [i-1] : [null]

    switch (token [TYPE]) {

      case AUTH:
        if (R_DRIVE.test (value._auth)) // TODO cleanup
          r.push ([AUTH, _emptyAuth], done = [DRIVE, value._auth])
        else
          r.push (token)
      break
      ...
      case DRIVE:
        if (!done)
          ...
          r.push ([ROOT, '/'], [FILE, value])
        else if (last [TYPE] === ROOT)
          r.push ([FILE, value])
      break

      case DIR:
        if (!done && R_DRIVE.test (value)) {
          if (last [TYPE] === ROOT) r.pop ()
          r.push (done = [DRIVE, value], [ROOT, '/'])
        }
        else
          r.push (done = token)
      break
