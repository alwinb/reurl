...
const SCHEME = 'scheme'
const AUTH   = 'authority' // 'opaque-auth'
const DRIVE  = 'drive'
const ROOT   = 'root'
const DIR    = 'directory'
const FILE   = 'file'
...
function printToken (token) {
  ...
  switch (token [TYPE]) {
    ...
    case AUTH: return '//' + auth.print (v)
    case DRIVE: return '/' + v
    case ROOT: return '/'
    case DIR: return wtf8.encode (v) .replace (PATH_ESC, _escapeChar) + '/'
    case FILE: return wtf8.encode (v) .replace (PATH_ESC, _escapeChar)
    case QUERY: return '?' + wtf8.encode (v) .replace (QUERY_ESC, _escapeChar)
    case FRAG: return '#' + wtf8.encode (v)
  }
}
...
function normalize (url) {
  ...
  for (let i = 0; i < url.length; i++) {
    ...
    switch (a [TYPE]) {
      ...
      case AUTH:
        r.push ([AUTH, auth.normalize (a [VALUE], scheme)])
      break
...
function letter (as) {
  ...
  for (let i=0, l=as.length; i<l; i++) {
    ...
    switch (a [TYPE]) {
      ...
      case AUTH:
        if (R_DRIVE.test (v._auth)) // TODO cleanup
          r.push ([AUTH, _emptyAuth], done = [DRIVE, v._auth])
        else
          r.push (a)
      break
