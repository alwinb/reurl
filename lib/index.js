...
class Url { 
  ...
  static fromIterable (fields, { percentCoded = false } = { }) {
  ...
  *tokens () {
  ...
  set (patch) {
    ...
    if ('host' in patch) {
      // TODO should this indeed be _after_ reconcile?; if patch is decoded, better do it before
...
function parseAuth (input, { _scheme, percentCoded }) {
  // log ('parseAuth', {input,  _scheme, percentCoded })
  ...
  if (host != null)
    host = parseHost (host, { percentCoded, opaque:!(_scheme in specialSchemes) })
  return { user, pass, host, port }
}
...
function parseUrl (input, type) {
  ...
  const { user, pass, host, port } = auth != null ? parseAuth (auth, { _scheme:type, percentCoding:'preserve', percentCoded:true } ) : { }
