...
class Url {
  ...
  static parseAndResolve (input, base) {
    try { base = new this (base) .forceResolve () }
    catch (e) { throw new Error ('ERR_INVALID_BASE_URL') }
    input = new this (input, { parser:base.scheme })
    return input.forceResolve (base) .normalise ()
  }
  ...
  toString ({ ascii = false } = { }) {
    ...
    const relative = !Core.isBase (this) // 'nonbase' URLs have less characters escaped in path
    const enc = encode ({ percentCoded, special, relative, ascii })
  ...
  set (patch) {
    ...
    const r = this.constructor .fromObjects (patch, reset, this)
  ...
  forceResolve (base) {
    return this.resolve (base) .force ()
  }
  ...
  resolve (base) {
    ...
    if (base != null) {
      ...
      if (Core.isBase (base) || this.hash != null)
        resolved = base .goto (this)
    }
    if (!resolved.scheme && !resolved.hash)
      throw new Error ('ERR_CANNOT_RESOLVE')
    return resolved
  }

  // Forces the presence of an authority
  // Force is only applied to special URLs

  force () {
    if (!this.scheme) throw new Error ('ERR_FORCE_NOSCHEME')
    if (Core.isSpecial (this) && !Core.hasSubstantialAuth (this)) {
      if (this._scheme === 'file')
        return this.constructor.fromObjects (markValid ({ host:'' }), this)
      const r = markValid ({ })
...
class Core {
  ...
  static isBase ({ scheme, host, root, drive }) {
    return scheme != null && (host != null || root || drive)
  }
