...
const modes =
  { regular: 0b100, special: 0b101, nonbase: 0b010 }
...
class Encoder {
  ...
  constructor ({ ascii=false, percentCoded=false, special=false, nonbase=false } = { }) {
    this.ascii = ascii
    this.percentCoded = percentCoded
    this.mode = nonbase ? modes.nonbase : special ? modes.special : modes.regular
  }
...
const encode = options => (value, key) => {
  ...
  return value == null ? null : coder.encode (String (value), key)
}
