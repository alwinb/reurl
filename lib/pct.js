...
// To convert an URL to an URL string, specific subsets of chars need to be
// converted to percent-escape-sequences. Such a subset is called the _escape set_
// for a given token, and not all token-types have the same escape set. 

// The collection of escape sets can be modelled as a function from char -> int;
// using bitfields. Given a char, the i-th bit from the right is an on-bit iff
// the i-th set contains the char.

// For example, given a collection of character sets { A1, .. An }, the 
// function shall return 0b1011 iff the char is present in exactly A1, A2 and A4. 

const tokenIds =
  { user:1<<1, pass:1<<2, host:1<<3, dir:1<<7, file:1<<8, query:1<<9, hash:1<<10 }
...
const encode = ({ ascii=false, percentCoded=false, special=false, nonbase=false } = { }) => (v, k) => {
  ...
  const out = []
  const escapeSet = (k in tokenIds ? tokenIds [k] : 0)
