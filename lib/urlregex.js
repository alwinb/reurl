...
const tokenTypes = ['scheme', 'auth', 'root', 'dir', 'file', 'query', 'hash']
const typeOrd    = { scheme:1, auth:2, root:4, dir:5, file:6, query:7, hash:8 }
...
class Url { 
  ...
  *tokens () {
  ...
  static needsRoot ({ host, dirs, file }) {
    return (host != null) && (dirs && dirs.length || file) ? '/' : null
  }
  ...
  static enforceConstraints (r) {
    if ((!r.host || r._scheme === 'file') && (r.user != null || r.port != null))
      throw new Error ('ERR_NOAUTH')

    else if (r.user == null && r.pass != null)
      throw new Error ('ERR_NOCREDS')

    if (r.drive && r.scheme && r._scheme !== 'file')
...
function parseUrl (input, type = 'http') {
  let [_, scheme, rest] = parsers.splitScheme.exec (input.replace (TRIM, ''))
  if (scheme) type = scheme.toLowerCase ()
