...
class Url { 
  ...
  constructor (input, { percentCoding = 'decode', parser = 'http' } = { }) {
    ...
    const $ = { value:null, enumerable:false, configurable:true }
    Object.defineProperties (this, { scheme:$, pass:$, user:$, host:$, port:$, drive:$, root:$, dirs:$, file:$, query:$, scheme_:$ })
    assign (this, 'percentCoded', percentCoded)
    ...
    const coded = percentCoded === input.percentCoded ? entries (input)
      : map (input, !percentCoded ? decode : _encode ())
    assemble (coded, this)
    return this.enforceConstraints ()
  }

  static fromIterable (fields, { percentCoded = false } = { }) { // TODO, pass percentCoding properly
    const percentCoding = percentCoded ? 'preserve' : 'decode'
    const r = assemble (fields, new Url (null, { percentCoding }))
    return r.enforceConstraints ()
  }
  ...
  *tokens () {
  ...
  set (patch) {
    ...
    // TODO should validate indeed be called _before_ the decode?
    try { patch = assemble (map (patch, validate)) }
      catch (e) { throw e }

    if (percentCoded !== this.percentCoded)
      patch = assemble (map (patch, this.percentCoded ? _encode () : decode ), { percentCoded: this.percentCoded })
  ...
  enforceConstraints () {
    if (!this.root && Url.needsRoot (this))
      assign (this, 'root', '/') // sets implicit root

    if (this.scheme)
      assign (this, '_scheme', this.scheme.toLowerCase (), false)

    if (this.dirs && !this.dirs.length)
      assign (this, 'dirs', null)

    const { user, pass, host, port, _scheme } = this
    ...
    if (this.drive && _scheme && _scheme !== 'file')
      throw new Error ('ERR_NONFILE_DRIVE')

    let v;
    if ((v = this.port) !== null && isNaN (v) || v < 0 || v > MAX_PORT)
...
const validators = {
  ...
  // NB careful, the third arg is at the moment, a referece to the _patch_
  host (v, k, { percentCoded }) { // does not decode
    // NB The ip6 parser must be run before percent decoding
    // so, set (host:'[::]', percentCoded:false }) is now ambiguous...
    const l = v.length
    if (v[0] === '[' && v[l-1] === ']')
      return `[${ ip6.normalize (v.substring(1, l-1)) }]`
    else
      return v
  },

  port (v) { return v === '' ? null : +v },
...
function* _force (url) { // NB assumes URL does not have an auth or drive
  ...
  for (let [k,v] of tokens)
    if (k === 'scheme') yield [k,v]
      ...
      yield ['auth', parseAuth (v, url._scheme)]
      if (k === 'dir') yield ['root', '/']
      yield* tokens
    }
}
...
function parseAuth (input, type) {
  let [_, user = null, pass = null, host, port = null] = authExp.exec (input)
  if (port != null) {
    if (port === '') port = null
    else if (/^[0-9]+$/ .test (port)) port = +port
    else throw new SyntaxError ('ERR_INVALID_PORT')
  }
  if (host != null)
    host = parseHost (host, { opaque:!(type in specialSchemes) })
  return { user, pass, host, port }
}
...
function parseUrl (input, type) {
  ...
  const { user, pass, host, port } = auth != null ? parseAuth (auth, type) : { }
  return { scheme, user, pass, host, port, drive, root, dirs, file, query, hash, percentCoded:true }
}
...
const tableC6 = /[\uFFF9-\uFFFD]/g
const forbidden  = /[\x00\x09\x0A\x0D #%/:<>?@[\\\]^]/g
const forbidden_ = /[\x00\x09\x0A\x0D #/:<>?@[\\\]^]/g

function parseHost (input, { opaque = true, percentCoded = true } = { }) {
  const l = input.length
  if (l === 0) return input

  if (input[0] === '[' && input[l-1] === ']')
    return `[${ ip6.normalize (input.substring(1, l-1)) }]`
    ...
    try { input = ip4.print (ip4.parse (input)) }
